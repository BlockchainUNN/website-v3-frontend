        ...
        <img src={previousicon} alt="previous" onClick={handlePrevious} className="absolute -left-4 z-10 top-[40%] cursor-pointer w-12 h-12 md:w-auto md:h-auto" />
          ...
          <div className="font-wallpoet h-[fit-content] px-6 py-4 flex flex-col gap-4 md:gap-0 md:flex-row items-center justify-center bg-blockchain-white rounded-b-md">
            <div className="text-center">
              ...
              <h1 className="text-[20px] md:text-[35px] text-semibold">{title}</h1>
            </div>
          </div>
          <img src={nexticon} alt="previous" onClick={handleNext} className="absolute -right-4 z-10 top-[40%] cursor-pointer w-12 h-12 md:w-auto md:h-auto" />
        </div>
      </div>

      <button className={`${theme ? "text-white" : "text-black"} my-12 border border-blockchain-green rounded-sm bg-transparent px-6 py-4 text-[24px] font-mono`}>
        Explore Events
      </button>
    </div>
  );
